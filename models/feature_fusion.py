...
class Feature_fusion(nn.Module):
    def __init__(self):
        ...
        self.upsample_384 = nn.Sequential(
            nn.ConvTranspose2d(384, 256, 4, 4),
            nn.ReLU(inplace=True),
            nn.BatchNorm2d(256)
        )

        self.upsample_192 = nn.Sequential(
            nn.ConvTranspose2d(192, 256, 2, 2),
            nn.ReLU(inplace=True),
            nn.BatchNorm2d(256)
        )

        # Fusion of all levels: 3 * 256 = 768 channels
    ...
    def _autocast_context(self, feature_tensor):
        use_local_amp = feature_tensor.is_cuda and not torch.is_autocast_enabled()
        if use_local_amp:
            return torch.autocast(device_type="cuda"), True
        return nullcontext(), False
    
    def forward(self, left_features, right_features):
        amp_context, use_local_amp = self._autocast_context(left_features[0])
        with amp_context:
        ...
        if use_local_amp:
            left_output = left_output.to(dtype=left_features[0].dtype)
            right_output = right_output.to(dtype=right_features[0].dtype)
